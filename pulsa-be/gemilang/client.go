...
func New(baseURL, memberID, pin, password string, timeout time.Duration) *Client {
	return &Client{base: otomax.New(baseURL, memberID, pin, password, timeout)}
}

func (c *Client) validate() error {
	return c.base.Validate("gemilang")
}
...
var (
	reR         = regexp.MustCompile(`R#([0-9]{4,})`)
	reT         = regexp.MustCompile(`T#([0-9]{3,})`)
	rePriceDash = regexp.MustCompile(`-\s*([0-9][0-9\.\,]*)\s*=`)
	reAccepted  = regexp.MustCompile(`akan\s+diproses|sedang\s+diproses|AKAN\s+DIPROSES|SEDANG\s+DIPROSES`)
)
...
func (c *Client) TrxNoSign(ctx context.Context, product string, qty int64, dest string, refid string) (acc TrxAccepted, httpStatus int, body string, err error) {
	...
	return TrxAccepted{RefID: rid, Ticket: tid, Body: body, Price: price}, httpStatus, body, nil
}
...
func LooksLikeSystemIssue(body string) bool {
	...
	if strings.Contains(s, "TIMEOUT") || strings.Contains(s, "GANGGUAN SERVER") || strings.Contains(s, "MAINTEN") || strings.Contains(s, "SIBUK") {
		return true
	}
