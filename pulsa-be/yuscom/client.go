...
func (c *Client) validate() error {
	return c.base.Validate("yuscom")
}

type TrxAccepted struct {
	RefID  string // R#refid
	Ticket string // T#xxxx
	Body   string // raw response
	Price  int64  // jika bisa diparse dari body / callback price
}

var (
	reR = regexp.MustCompile(`R#([0-9]{6,})`)
	reT = regexp.MustCompile(`T#([0-9]{3,})`)
	// contoh: "Saldo 10.000.262 - 10.070 = 9.990.192" -> ambil 10070
	rePriceDash = regexp.MustCompile(`-\s*([0-9][0-9\.\,]*)\s*=`)

	// indikasi diterima untuk diproses
...
// Heuristik: kapan kita anggap Yuscom “bermasalah” (layak fallback ke JavaPay).
// - network error (err != nil)
// - http status bukan 200
// - body kosong / tidak ada pattern yang masuk akal
// - body mengandung kata yang mengindikasikan sistem down
func LooksLikeSystemIssue(body string) bool {
	...
	// kalau bukan "R#... akan diproses ..." dan bukan juga error bisnis yang jelas,
	// kita treat sebagai issue agar aman fallback.
	if !reAccepted.MatchString(s) && !strings.Contains(s, "GAGAL") && !strings.Contains(s, "SUKSES") {
		// contoh: "Invalid PIN or Password" ini juga issue config, fallback tidak menolong,
		// tapi tetap dikategorikan issue.
		return true
	}
	return false
}
