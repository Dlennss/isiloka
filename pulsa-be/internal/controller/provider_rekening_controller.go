...
type ProviderRekeningController struct {
	svc *service.ProviderRekeningService
}

func NewProviderRekeningController(svc *service.ProviderRekeningService) *ProviderRekeningController {
	return &ProviderRekeningController{svc: svc}
}

func (h *ProviderRekeningController) Handle(w http.ResponseWriter, r *http.Request) {
	base := "/v1/admin/provider-accounts"
	if r.URL.Path == base {
		switch r.Method {
		case http.MethodGet:
		...
		case http.MethodPost:
			var in repository.ProviderRekeningUpsertInput
	...
	case http.MethodGet:
		...
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				helper.WriteJSON(w, http.StatusNotFound, commondto.MapError("rekening provider not found"))
				return
			}
