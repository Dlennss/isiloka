...
func (s *ProviderCallbackService) processGemilangAppCallback(ctx context.Context, refid string, statusNum int, price int64, msg string, q url.Values) (int, map[string]any) {
	row, err := s.appProviderRepo.GetByRefID(ctx, refid, "gemilang")
	if err != nil {
	...
	if order.Status == "success" || order.Status == "failed" || order.Status == "refunded" {
		helper.AppendAlreadyFinalLog("callback already_final provider=gemilang refid=%s app_order_id=%d status=%s", refid, order.ID, order.Status)
		return 200, map[string]any{"ok": true, "already_final": true, "refid": refid}
	}

	finalStatus := resolveGemilangFinalStatus(statusNum, strconv.Itoa(statusNum), msg)
	...
	_, snParsed := providersn.ParseGemilangSNRefFromMsg(msg)
	snParsed = strings.TrimSpace(snParsed)
	rcStr := strconv.Itoa(statusNum)
	rawJSON, _ := json.Marshal(map[string]any{"t": q.Get("t"), "refid": refid, "status": statusNum, "price": price, "message": msg})
	updateIn := repository.AppOrderProviderTrxUpdateInput{ID: row.ID, Status: status, KodeRespon: rcStr, Pesan: msg, SN: snParsed, RawCallback: string(rawJSON)}
	if price > 0 {
		updateIn.HargaProvider = &price
	}
	if err := s.appProviderRepo.UpdateResult(ctx, updateIn); err != nil {
		helper.AppendProviderServiceLog("provider_callback_service.log", "gemilang app callback update failed refid=%s app_provider_id=%d err=%v", refid, row.ID, err)
	}
	...
	if status == "success" {
		if price > 0 {
			...
			if _, _, err := s.repo.ApplyProviderWalletTx(ctx, repository.CallbackProviderWalletTxIn{Provider: "gemilang", RefID: refid, Arah: "debit", Jumlah: price, Alasan: "APP_TRX_SUCCESS_COST", Catatan: "auto debit by callback (app success)", AppOrderProviderTrxID: &appProviderID}); err != nil {
				helper.AppendProviderServiceLog("provider_wallet.log", "provider wallet debit app failed provider=gemilang refid=%s app_provider_id=%d err=%v", refid, row.ID, err)
			}
		}
