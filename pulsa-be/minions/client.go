...
func (c *Client) validate() error {
	if strings.TrimSpace(c.BaseURL) == "" || strings.TrimSpace(c.MemberID) == "" {
		return errors.New("minions config missing (baseurl/memberid)")
	}
	if strings.TrimSpace(c.PIN) == "" || strings.TrimSpace(c.Password) == "" {
		return errors.New("minions config missing (pin/password)")
	}
	return nil
}
