...
type UserInvoiceCheckerProps = {
  isLoggedIn?: boolean;
};

export function UserInvoiceChecker({ isLoggedIn = false }: UserInvoiceCheckerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/user/transaksi/${invoiceId.trim()}?${qs.toString()}`);
    } catch (error) {
